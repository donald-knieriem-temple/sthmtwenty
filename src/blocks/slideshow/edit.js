...
import Slidenav from '../../../templates/components/slidenav/slidenav.js';
import Dotnav from '../../../templates/components/dotnav/dotnav.js';
/**
...
const Edit = ( props ) => {
	...
	var { 
		...
		classList,
		clientId,
		attributes: { anchor, animation, autoplay, finite, ratio, minHeight, maxHeight, hasNav, hasDotnav, numItems },
		setAttributes,
	} = props;
    ...
    setAttributes({ numItems: innerCount });
	...
	return (
		<>
			<Slideshow
				...
				hasNav={ hasNav} hasDotnav={ hasDotnav } numItems={ numItems }>
				Slideshow #{ anchor } ({ numItems} slides)
			...
			<PanelBody title={ __('Slideshow Options', 'sthmtwenty') }>
				...
				<ToggleControl
			        label={ __( 'Nav Arrows' ) }
			        help={ hasNav ? 'Has Nav Arrows' : 'Does Not Have Nav Arrows' }
			        checked={ hasNav }
			        onChange={ ( newHasNav ) => setAttributes( { hasNav: newHasNav, } ) }
			    />
			    <ToggleControl
			        label={ __( 'Dot Nav' ) }
			        help={ hasDotnav ? 'Has Dot Nav' : 'No Dot Nav' }
			        checked={ hasDotnav }
			        onChange={ ( newHasDotnav ) => setAttributes( { hasDotnav: newHasDotnav, } ) }
			    />
			</PanelBody>
