 ...
 */
function sthm_section_block_init() {
	$dir = dirname( __FILE__ );

	$script_asset_path = "$dir/build/index.asset.php";
	//$script_dir = "$dir/build/";
	$script_dir_uri = get_stylesheet_directory_uri() . "/blocks/section/";
	...
	if ( ! file_exists( $script_asset_path ) ) {
		throw new Error(
			'You need to run `npm start` or `npm run build` for the "sthm/section" block first.'
		);
	}
	$index_js     = 'build/index.js';
	$script_asset = require( $script_asset_path );
	wp_register_script(
		'sthm-section-block-editor',
	...
	register_block_type( 'sthm/section', array(
		'editor_script' => 'sthm-section-block-editor',
		'editor_style'  => 'sthm-section-block-editor',
		'style'         => 'sthm-section-block',
	) );
}

//add_action( 'init', 'sthm_section_block_init' );
sthm_section_block_init();